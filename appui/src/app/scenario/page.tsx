...
export default function Page() {
  ...
  const selectedFormat = (watch("battleFormat") as keyof typeof FORMAT_MATRIX) ?? "1v1";
  ...
  // Helpers to update a player cell
  ...
  const onSubmit = async (values: ScenarioInputForm) => {
    ...
    try {
      ...
      const t = setTimeout(
        () => ctrl.abort(),
        Number(process.env.NEXT_PUBLIC_GENERATE_TIMEOUT_MS || 120000)
      );

      // Build Warhost structure for the generator from UI selections
      ...
      setOutput(`Request failed.\n\nSee Network tab for /api/generate.\n${String(err)}`);
      setAiUsed(false);
        ...
        <div className="mt-3">
          {aiUsed !== null && (
            <div className="mb-2 text-sm">
              {aiUsed ? (
                <span className="rounded-md bg-green-600/20 px-2 py-1 text-green-200">AI enhanced</span>
              ) : (
                <span className="rounded-md bg-yellow-600/20 px-2 py-1 text-yellow-200" title={aiError ?? undefined}>
                  Template fallback
                </span>
              )}
            </div>
          ...
          <div className="flex gap-2">
            <Link href="/saved" className="inline-block">
              <Button className="bg-white text-black hover:bg-white/90">View in Saved</Button>
            </Link>
            <Button
